...
def desenhaCabecalho(img, largura, altura, aluno, prof, prova, turma, data, nota_prova):
    ...
    """
    aluno = safeText(aluno)
    prof = safeText(prof)
    prova = safeText(prova)
    turma = safeText(turma)
    data = safeText(data)
...
def getOurSqrV2(im_color, expected_questions = None, expected_alternatives = None):
    def getCenterOfMass(square):
        return (square[0][0]+square[1][0]+square[2][0]+square[3][0])/4, (square[0][1]+square[1][1]+square[2][1]+square[3][1])/4

    def cluster_candidates(items, axis, tolerance):
        groups = []
        for item in sorted(items, key=lambda candidate: candidate['center'][axis]):
            position = item['center'][axis]
            if (not groups) or abs(position - groups[-1]['mean']) > tolerance:
                groups.append({'items': [item], 'mean': position})
            else:
                groups[-1]['items'].append(item)
                groups[-1]['mean'] = sum(candidate['center'][axis] for candidate in groups[-1]['items']) / len(groups[-1]['items'])
        return groups

    def dedupe_by_axis(items, axis, tolerance):
        deduped = []
        for item in sorted(items, key=lambda candidate: candidate['center'][axis]):
            if (not deduped) or abs(item['center'][axis] - deduped[-1]['center'][axis]) > tolerance:
                deduped.append(item)
            elif item['area'] > deduped[-1]['area']:
                deduped[-1] = item
        return deduped

    def fallback_alternatives(items, question_items, tolerance, median_size_value):
        if len(question_items) == 0:
            return []

        top_question_y = min(item['center'][1] for item in question_items)
        left_question_x = min(item['center'][0] for item in question_items)
        vertical_limit = top_question_y + (median_size_value * 1.2)
        horizontal_limit = left_question_x + (median_size_value * 1.5)

        filtered = []
        for item in items:
          if item['center'][1] <= vertical_limit and item['center'][0] >= horizontal_limit:
              filtered.append(item)

        filtered = dedupe_by_axis(filtered, 0, tolerance)
        filtered.sort(key=lambda item: item['center'][0])
        return filtered

    def synthesize_alternatives(question_items, expected_count, marker_size):
        if len(question_items) < 2 or expected_count is None or expected_count <= 0:
            return []

        vertical_distances = []
        for i in range(len(question_items) - 1):
            vertical_distances.append(question_items[i + 1]['center'][1] - question_items[i]['center'][1])
        avg_question_spacing = sum(vertical_distances) / len(vertical_distances)

        question_center_x = sum(item['center'][0] for item in question_items) / len(question_items)
        top_question_y = min(item['center'][1] for item in question_items)

        horizontal_spacing = avg_question_spacing * (120.0 / 45.0)
        first_alt_x = question_center_x + (140.0 / 45.0) * avg_question_spacing
        alt_y = top_question_y - (50.0 / 45.0) * avg_question_spacing
        half_size = max(4.0, marker_size / 2.0)

        synthesized = []
        for index in range(expected_count):
            center_x = first_alt_x + (index * horizontal_spacing)
            center_y = alt_y
            square = [
    ...
    for cnt in contours:
        ...
        square = formatShape([approx])[0]
        center = getCenterOfMass(square)

        if center[0] / width < 0.02 or center[1] / height < 0.02:
            continue
        if (width - center[0]) / width < 0.02 or (height - center[1]) / height < 0.02:
            continue
    ...
    median_size = float(np.median([candidate['size'] for candidate in candidates]))
    filtered_candidates = []
    for candidate in candidates:
        if 0.6 * median_size <= candidate['size'] <= 1.6 * median_size:
            filtered_candidates.append(candidate)
    candidates = filtered_candidates if len(filtered_candidates) > 0 else candidates

    position_tolerance = max(8, int(median_size * 0.8))
    dedupe_tolerance = max(6, int(median_size * 0.6))

    x_clusters = cluster_candidates(candidates, 0, position_tolerance)
    y_clusters = cluster_candidates(candidates, 1, position_tolerance)

    question_cluster = max(
        x_clusters,
        key=lambda group: (
            len(group['items']),
            max(item['center'][1] for item in group['items']) - min(item['center'][1] for item in group['items'])
        )
    )
    alternative_cluster = max(
        y_clusters,
        key=lambda group: (
            len(group['items']) >= 2,
            -(group['mean']),
            max(item['center'][0] for item in group['items']) - min(item['center'][0] for item in group['items']),
            len(group['items'])
        )
    )

    question_items = dedupe_by_axis(question_cluster['items'], 1, dedupe_tolerance)
    alternative_items = dedupe_by_axis(alternative_cluster['items'], 0, dedupe_tolerance)

    fallback_items = fallback_alternatives(candidates, question_items, dedupe_tolerance, median_size)
    if len(fallback_items) > len(alternative_items):
        alternative_items = fallback_items

    if expected_alternatives is not None and len(alternative_items) != expected_alternatives and len(fallback_items) > 0:
        alternative_items = fallback_items

    if expected_alternatives is not None and len(alternative_items) != expected_alternatives:
        synthesized_items = synthesize_alternatives(question_items, expected_alternatives, median_size)
        if len(synthesized_items) == expected_alternatives:
            alternative_items = synthesized_items

    question_items.sort(key=lambda item: item['center'][1])
    alternative_items.sort(key=lambda item: item['center'][0])

    question_squares = [[item['square'], item['center']] for item in question_items]
    alternative_squares = [[item['square'], item['center']] for item in alternative_items]

    return question_squares, alternative_squares, im_crop



def getAnswers(img, expected_questions = None, expected_alternatives = None):
    def getProportion(a, b):
...
def getAnswersV2(img, expected_questions = None, expected_alternatives = None):
    def getProportion(a, b):
        return abs(a-b)/max(a,b)

    def build_circle_data(contours):
        circles = []
        for contour, center in contours:
            ...
            for point in contour:
                point_x = point[0][0]
                point_y = point[0][1]
                distances.append(math.sqrt((point_x - center_x)**2 + (point_y - center_y)**2))
            if len(distances) == 0:
                continue
        ...
        circles.sort(key=lambda item: item['area'])
        return circles
    ...
    for cnt in contours:
        ...
        if len(approx) > 4:
            ...
            raw_circles.append([approx, [center_x, center_y]])

    circles = build_circle_data(raw_circles)
    if len(circles) == 0:
        return []

    for i in range(len(circles)-1, -1, -1):
        if (circles[i]['center'][0] <= x0 + int(x0 * 0.7)) or (circles[i]['center'][0] >= x1) or (circles[i]['center'][1] <= y0) or (circles[i]['center'][1] >= y1 + int(y1 * 0.07)):
            del circles[i]
    ...
    for i in range(len(circles)-1, -1, -1):
        if len(circles) <= 1:
            break
        if i == len(circles)-1:
            if getProportion(circles[i]['area'], circles[i-1]['area']) > 0.2:
                del circles[i]
        else:
            if getProportion(circles[i]['area'], circles[i+1]['area']) > 0.2:
                del circles[i]
    ...
    for circle in circles:
        center_x = int(circle['center'][0])
        center_y = int(circle['center'][1])
        radius = max(4, int(circle['radius'] * 0.55))

        mask = np.zeros(im_gray.shape, dtype=np.uint8)
        cv2.circle(mask, (center_x, center_y), radius, 255, -1)
        pixels = im_gray[mask == 255]
        if len(pixels) == 0:
            continue

        circle['darkness'] = 255 - float(np.mean(pixels))
        if circle['darkness'] >= 55:
            filled_circles.append(circle)

    threshold = 0.055
    letters = [i + 1 for i in range(8)]
    gabarito_aluno = []

    for question_index in range(len(n)):
        row_candidates = []
        for circle in filled_circles:
            if getProportion(n[question_index][1][1], circle['center'][1]) < threshold:
                row_candidates.append(circle)

        if len(row_candidates) == 0:
            gabarito_aluno.append((question_index + 1, 0))
            continue

        row_candidates.sort(key=lambda candidate: candidate['darkness'], reverse=True)
        selected_circle = row_candidates[0]
        selected_option = 0

        for option_index in range(len(alt)):
            if getProportion(alt[option_index][1][0], selected_circle['center'][0]) < threshold:
                selected_option = letters[option_index]
                break

        gabarito_aluno.append((question_index + 1, selected_option))
